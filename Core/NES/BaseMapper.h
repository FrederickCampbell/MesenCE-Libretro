...
public:
	...
	// Return the currently mapped writable 256-byte CPU page for Libretro's
	// memory descriptor API. The pointer is the page base, matching Read(addr)'s
	// _prgPages[addr >> 8][(uint8_t)addr] layout.
	uint8_t* GetLibretroCpuRamPage(uint16_t addr)
	{
		if(addr < 0x6000) {
			return nullptr;
		}
		uint8_t page = (uint8_t)(addr >> 8);
		if((_prgMemoryAccess[page] & MemoryAccessType::Write) && _prgPages[page]) {
			return _prgPages[page];
		}
		return nullptr;
	}

	uint8_t* GetLibretroSaveRam() { return _saveRam; }
	uint32_t GetLibretroSaveRamSize() const { return _saveRamSize; }
