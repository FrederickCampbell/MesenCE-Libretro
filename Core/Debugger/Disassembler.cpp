...
void Disassembler::GetLineData(DisassemblyResult& row, CpuType type, MemoryType memType, CodeLineData& data)
{
	...
	data.Value = 0;
	data.LineCpuType = CpuType::Nes;
	data.Text[0] = 0;
	data.Comment[0] = 0;

	if(type != CpuType::Nes || row.Address.Address < 0) {
		return;
	}

	if(data.Flags & (LineFlags::BlockStart | LineFlags::BlockEnd | LineFlags::Empty)) {
		return;
	}
	...
	if(data.Flags & LineFlags::ShowAsData) {
		...
		for(int i = 0; i < row.GetByteCount(); i++) {
			str.Write(" $", 2);
			str.Write(HexUtilities::ToHexChar(
				_memoryDumper->GetMemoryValue(memType, row.CpuAddress + i)
			), 2);
		}
	...
	if(data.Flags & LineFlags::Label) {
		string label = _labelManager->GetLabel(row.Address);
		if(label.empty()) {
			label = "$" + DebugUtilities::AddressToHex(CpuType::Nes, row.CpuAddress);
		}
		label += ":";
		data.Flags |= LineFlags::VerifiedCode;
		memcpy(data.Text, label.c_str(), std::min((int)label.size() + 1, 1000));
		return;
	}

	DisassemblerSource& src = GetSource(row.Address.Type);
	DisassemblyInfo disInfo = src.Cache[row.Address.Address];
	CodeDataLogger* cdl = _debugger->GetCdlManager()->GetCodeDataLogger(row.Address.Type);
	if(!disInfo.IsInitialized()) {
		disInfo = DisassemblyInfo(
			row.Address.Address, 0, CpuType::Nes, row.Address.Type, _memoryDumper
		);
	} else {
		data.Flags |= (!cdl || cdl->IsCode(data.AbsoluteAddress.Address))
	...
	string output;
	disInfo.GetDisassembly(output, row.CpuAddress, _labelManager, _settings);
	memcpy(data.Text, output.c_str(), std::min((int)output.size() + 1, 1000));
	disInfo.GetByteCode(data.ByteCode);

	if(data.Flags & LineFlags::Comment) {
		string comment = ";" + _labelManager->GetComment(row.Address);
		memcpy(data.Comment, comment.c_str(), std::min((int)comment.size() + 1, 1000));
	}
}
