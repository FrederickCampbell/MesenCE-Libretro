...
// MESENCE_LIBRETRO_RA_HD_PATCH_2026_06_30
static std::array<retro_memory_descriptor, 260> _raMemoryDescriptors = {};
static uint64_t _libretroRunCounter = 0;
static uint64_t _libretroPresentedFrames = 0;
static uint64_t _libretroEmptyFrames = 0;
static uint64_t _libretroDuplicatedFrames = 0;
...
static void ConfigureRetroArchFolders()
{
	...
	env_cb(RETRO_ENVIRONMENT_GET_SYSTEM_DIRECTORY, &systemDir);
	env_cb(RETRO_ENVIRONMENT_GET_SAVE_DIRECTORY, &saveDir);

	if(systemDir && systemDir[0]) {
		// GetHdPackFolder() becomes <RetroArch system>/HdPacks and firmware
		// lookup points at the same system directory.
		FolderUtilities::SetHomeFolder(systemDir);
	}

	string save = (saveDir && saveDir[0]) ? string(saveDir) : string();
	string firmware = (systemDir && systemDir[0]) ? string(systemDir) : string();
	FolderUtilities::SetFolderOverrides(save, save, save, firmware);
}

static void PublishRetroAchievementsMemoryMap()
{
	if(!env_cb || !_console || !_console->GetMemoryManager() || !_console->GetMapper()) {
		return;
	}

	size_t count = 0;
	auto addDescriptor = [&](void* ptr, size_t start, size_t length) {
		if(!ptr || length == 0 || count >= _raMemoryDescriptors.size()) {
			return;
		}
		retro_memory_descriptor& d = _raMemoryDescriptors[count++];
		d = {};
		d.ptr = ptr;
		d.start = start;
		d.len = length;
	...
	// NES 2 KiB internal RAM and its three CPU-bus mirrors.
	uint8_t* internalRam = _console->GetMemoryManager()->GetInternalRam();
	addDescriptor(internalRam, 0x0000, 0x0800);
	addDescriptor(internalRam, 0x0800, 0x0800);
	addDescriptor(internalRam, 0x1000, 0x0800);
	addDescriptor(internalRam, 0x1800, 0x0800);

	// Publish every currently mapped writable mapper page. This covers ordinary
	// cartridge WRAM/SRAM and the writable CPU pages used by FDS.
	BaseMapper* mapper = _console->GetMapper();
	for(uint32_t addr = 0x6000; addr <= 0xFF00; addr += 0x100) {
		addDescriptor(mapper->GetLibretroCpuRamPage((uint16_t)addr), addr, 0x100);
	}

	retro_memory_map map = {};
	map.descriptors = _raMemoryDescriptors.data();
	map.num_descriptors = (unsigned)count;
	env_cb(RETRO_ENVIRONMENT_SET_MEMORY_MAPS, &map);

	bool supportsAchievements = true;
	env_cb(RETRO_ENVIRONMENT_SET_SUPPORT_ACHIEVEMENTS, &supportsAchievements);
}
...
extern "C" {
	...
	if(_emu) {
		...
		// Emulator::Stop() drops MesenCE's internal safe_ptr, but this Libretro
		// frontend owns a second shared_ptr. Destroy the console while the
		// Emulator and EmuSettings still exist. FDS destructors touch settings.
		_console.reset();
...
*/
	...
	bool hasThrottleState = env_cb
		&& env_cb(RETRO_ENVIRONMENT_GET_THROTTLE_STATE, &throttleState);
	bool frontendFastForward = hasThrottleState
		&& throttleState.mode == RETRO_THROTTLE_FAST_FORWARD;

	if(logCallback && hasThrottleState && throttleState.mode != _libretroLastThrottleMode) {
		logCallback(
			RETRO_LOG_INFO,
			"[MesenCE][Pacing] mode=%u rate=%.3f fastForward=%d owner=RetroArch\n",
			(unsigned)throttleState.mode,
			(double)throttleState.rate,
			frontendFastForward ? 1 : 0
		);
		_libretroLastThrottleMode = throttleState.mode;
	}
	...
	if(_emu && _emu->GetConsole()) {
		...
		// end-of-frame bookkeeping exactly once for each retro_run().
		if(_emu->GetRewindManager()) {
			_emu->GetRewindManager()->ProcessEndOfFrame();
		}
		...
		// frontend fast-forward, never block emulation on an expensive HD decode;
		// VideoDecoder drops incoming video frames while its worker is busy.
		if(decoder && !frontendFastForward) {
			decoder->WaitForAsyncFrameDecode();
		}

		RenderedFrame frame = {};
		if(renderer) {
			frame = renderer->GetLastFrame();
		}

		bool validFrame = frame.FrameBuffer && frame.Width > 0 && frame.Height > 0;
		bool newDecodedFrame = validFrame && (
			!_libretroHasSubmittedFrame
			|| frame.FrameNumber != _libretroLastSubmittedFrameNumber
		);
		bool submitDecodedFrame = validFrame && (!frontendFastForward || newDecodedFrame);

		if(submitDecodedFrame) {
			_libretroPresentedFrames++;

			if(_geometryDirty || frame.Width != _lastReportedWidth || frame.Height != _lastReportedHeight) {
				retro_game_geometry geometry = {};
				geometry.base_width = frame.Width;
				geometry.base_height = frame.Height;
				geometry.max_width = 2560;
				geometry.max_height = 2560;
				geometry.aspect_ratio = 4.0f / 3.0f;
				env_cb(RETRO_ENVIRONMENT_SET_GEOMETRY, &geometry);
				_lastReportedWidth = frame.Width;
				_lastReportedHeight = frame.Height;
			...
			_libretroLastSubmittedFrameNumber = frame.FrameNumber;
			_libretroHasSubmittedFrame = true;
		} else {
			if(!validFrame) {
				_libretroEmptyFrames++;
			}
			_libretroDuplicatedFrames++;
			// A null buffer asks the frontend to duplicate the previous frame.
			// This is expected while fast-forward outruns HD texture decoding.
			if(_videoRefresh) {
		...
		if(logCallback && (
			_libretroRunCounter <= 8
			|| _libretroPresentedFrames == 1
			|| (_libretroRunCounter % 300) == 0
		)) {
			logCallback(
				RETRO_LOG_INFO,
				...
				frontendFastForward ? 1 : 0,
				(unsigned)_emu->GetFrameCount(),
				(unsigned)(decoder ? decoder->GetFrameCount() : 0),
...
*/	}
	...
	if(logCallback) {
		...
		logCallback(
			RETRO_LOG_INFO,
			"[MesenCE][Paths] firmware=%s hdPacks=%s\n",
			firmwareFolder.c_str(),
			hdPackFolder.c_str()
		);
	}
...
*/
		...
		if(result) {
			...
			if(_console && _console->GetHdData()) {
				...
				env_cb(RETRO_ENVIRONMENT_SET_GEOMETRY, &avInfo.geometry);
			}
	...
	*/
	...
	uint32_t width = _lastReportedWidth ? _lastReportedWidth : 256;
	uint32_t height = _lastReportedHeight ? _lastReportedHeight : 240;
	if(_emu && _emu->GetVideoDecoder()) {
	...
	info->geometry.max_width = 2560;
	info->geometry.max_height = 2560;
	info->geometry.aspect_ratio = 4.0f / 3.0f;
	info->timing.fps = _emu ? _emu->GetFps() : 60.0988;
	info->timing.sample_rate = _audioSampleRate;
}
	...
	if(id == RETRO_MEMORY_SYSTEM_RAM) {
		return _console->GetMemoryManager() ? _console->GetMemoryManager()->GetInternalRam() : nullptr;
	}
	if(id == RETRO_MEMORY_SAVE_RAM) {
		return _console->GetMapper() ? _console->GetMapper()->GetLibretroSaveRam() : nullptr;
	}
	return nullptr;
}
	...
	if(id == RETRO_MEMORY_SAVE_RAM) {
		return _console->GetMapper() ? _console->GetMapper()->GetLibretroSaveRamSize() : 0;
	}
	return 0;
}
}
